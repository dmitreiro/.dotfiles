...
def on_connect(_):
    dialog = Gtk.Dialog(title="UA VPN Connect", flags=0)
    dialog.set_default_size(320, -1)

    vbox = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=6)
    vbox.set_border_width(15)

    label = Gtk.Label(label="Enter VPN password:")
    label.set_xalign(0)
...
def check_process():
    ...
    if connected:
        ...
        indicator.set_title("UA VPN Connected")

        if last_status is not True:
            Notify.Notification.new("UA VPN Status", "Connected", "network-vpn-symbolic").show()
        ...
        indicator.set_title("UA VPN Disconnected")

        if last_status is not False:
            Notify.Notification.new("UA VPN Status", "Disconnected", "network-vpn-disconnected-symbolic").show()
